...
### Added
- Homebrew (macOS) tap: `brew tap skhell/pingtrace && brew install pingtrace`.
- `.deb` and `.rpm` packages available on every GitHub Release for Debian/Ubuntu and Fedora/RHEL.
- GoReleaser pipeline extended to auto-publish the Homebrew formula on every tag.
